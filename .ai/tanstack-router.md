...
function Navigation() {
  return (
    <>
      ...
      {/* With hash and state */}
      <Link to="/docs" hash="#introduction" state={{ fromPage: 'home' }}>
        Documentation
      </Link>
...
function FilterSidebar() {
  ...
  return (
    <div>
      <Link to="/products" search={{ ...search, filter: 'electronics' }}>
        Electronics
      </Link>
    </div>
  );
...
function Navigation() {
  ...
  return (
    <nav>
      <Link to="/">Home {isHomepage && '←'}</Link>
      <Link to="/posts">Posts {isPostsSection && '←'}</Link>
...
export const Route = createFileRoute('/search')({
  validateSearch: (search: Record<string, unknown>) => {
    return {
      q: String(search.q || ''),
      page: Number(search.page || 1),
      filters: Array.isArray(search.filters) ? search.filters.map(String) : [],
    };
  },
});
```
...
export const Route = createFileRoute('/posts')({
  ...
  loader: ({ context: { queryClient }, deps }) =>
    queryClient.ensureQueryData(postsQueryOptions(deps.page, deps.filter)),
  component: PostsList,
});
